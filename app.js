...
function render() {
  ...
  if (worldStore.user) {
    ...
    html += '</div>';
    html += 'Logged in as <code>' + worldStore.user.uname+ '</code> with <code>' + helpers.commafy(helpers.round10(worldStore.user.balance/100, -20)) + '</code> bits' ;
    $('#user-info').html(html);
  } else {
    $('#user-info').empty();
  }
  ...
  if (worldStore.user && !worldStore.claimedAt && grecaptcha) {
    ...
    if (worldStore.faucetResult === 'FAUCET_SUCCESS') {
      html += '<span class="label label-success">+20 bits</span> ';
